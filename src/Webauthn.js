...
class Webauthn {
  ...
  register(options = {}) {
    ...
    return async (req, res, next) => {
      ...
      if (
        ...
        existing.authenticators.length > 0
      ) {
  ...
  login(options = {}) {
    ...
    return async (req, res, next) => {
      ...
      try {
        ...
        const assertionBuild = new AssertionChallengeBuilder(this);

        user.authenticators.map(authenticator => {
          assertionBuild.addAllowedCredential({ id: authenticator.credID });
        });

        const assertion = assertionBuild.build({ status: "ok" });
    ...
    return async (req, res, next) => {
      ...
      try {
        if (response.attestationObject !== undefined) {
          ...
          if (result.verified) {
            user.authenticators = [...user.authenticators, result.authrInfo];
            await this.store.put(username, user);
          }
        } else if (response.authenticatorData !== undefined) {
          result = user.authenticators.reduce((accepted, authenticator) => {
            if (accepted) {
              return accepted;
            }

            const result = Webauthn.verifyAuthenticatorAssertionResponse(
              response,
              authenticator
            );

            if (result.verified) {
              if (result.counter <= authenticator.counter) return false;
          ...
          if (result && result.verification.verified) {
            if (result.verification.counter <= result.authenticator.counter)
              throw new Error("Authr counter did not increase!");

            user.authenticators = user.authenticators.map(authenticator =>
              authenticator.publicKey === result.authenticator.publicKey
                ? { ...authenticator, counter: result.verification.counter }
                : authenticator
            );
            await this.store.put(username, user);
          }
  ...
  static verifyAuthenticatorAssertionResponse(webauthnResponse, authr) {
    ...
    if (["fido-u2f"].includes(authr.fmt)) {
      const authrDataStruct = Webauthn.parseGetAssertAuthData(
        authenticatorData
      );
      console.log("AUTH_DATA", authrDataStruct);
